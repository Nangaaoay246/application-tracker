...
import "@/lib/models";
import { Board, Column, JobApplications } from "@/lib/models";
import jobApplications from "@/lib/models/jobApplications";

const USER_ID = "6a8433446672e81101b30086";
...
async function seed() {
  ...
  try {
    ...
    const existingJobs = await JobApplications.find({ userId: USER_ID });
    if (existingJobs.length > 0) {
      console.log(
        `🗑️  Deleting ${existingJobs.length} existing job applications...`
      );
      await jobApplications.deleteMany({ userId: USER_ID });
